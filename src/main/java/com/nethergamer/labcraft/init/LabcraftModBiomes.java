...
import com.nethergamer.labcraft.world.biome.MoonBiomeBiome;
import com.nethergamer.labcraft.LabcraftMod;

import com.mojang.datafixers.util.Pair;

@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD)
public class LabcraftModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, LabcraftMod.MODID);
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource()instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(MoonBiomeBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, MOON_BIOME.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1, overworldRule(ResourceKey.create(Registry.BIOME_REGISTRY, MOON_BIOME.getId()),
									LabcraftModBlocks.MOON_STONE.get().defaultBlockState(), LabcraftModBlocks.MOON_STONE.get().defaultBlockState()));
